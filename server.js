// server.js — Ventura Jewelry chatbot (Final Enhanced Version)
...
const generic = new Set(["hola","buenas","dias","tardes","noches","hey","hello","hi","??","???","ver","mostrar","ensename","enseñame","enséname","foto","fotos","imagen","imagenes","imágenes","tiene","tienen","hay","disponible","disponibles"]);
const optional = new Set(["10k","14k","18k","24k","kt","k","g","gr","gramos","mm","cm","in","inch","pulgada","pulgadas","largo","ancho","peso","talla"]);
...
app.post("/chat", async (req, res) => {
  try {
    ...
    const needsHuman = /\b(problema|duda|whatsapp|hablar con|vendedora|humano|asesor|representante)\b/.test(norm);
    if (needsHuman) {
      return res.json({
        type: "collection",
    ...
    // RAG fallback with Pinecone context
    ...
    const enriched = [
      {
        role: "system",
        content: `Eres un asistente para una joyería llamada Ventura Jewelry en Miami.
Responde siempre en español, de manera amable y clara. Usa el contenido proporcionado para responder preguntas sobre productos, métodos de envío, devoluciones, garantías, y financiamiento con ShopPay, Klarna y Affirm.
Si no encuentras la información exacta, recomienda escribir al WhatsApp`
      },
      ...msgs
    ];
